...
func NewProfileHandlerInit(DB *gorm.DB, validator pkg.Validator, ulid pkg.UlidPkg, encryptor helper.Encryptor, jwt pkg.JwtC) ProfileHandlerInit {
	userRepo := userRepo.NewUserRepo(DB)
	uc := uc_profile.NewProfileUsecase(userRepo, encryptor)
	ucUser := userUsecase.NewUserUsecase(userRepo, ulid, encryptor)
	ucAuth := uc_auth.NewAuthUsecase(userRepo, encryptor, jwt)
...
func (d *profileHandlerInit) ProfileHandler() handler_profile.ProfileHandler {
	h := handler_profile.NewProfileHandler(d.uc.uc, d.uc.ucUser, d.uc.ucAuth, d.validator)
	return h
}
